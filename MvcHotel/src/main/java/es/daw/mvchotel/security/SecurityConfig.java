package es.daw.mvchotel.security;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http //al trabajar con un api rest sin estado. No hay frontend. No aplicamos protección csrf (tema sesiones). No aplicamos sesiones, sino tokens
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/", "/home", "/error", "/h2-console/**", "/login").permitAll()
                        ...
                        .requestMatchers("/funcionalidades").authenticated()
                        .requestMatchers("/habitacion/borrar/{id}","/habitacion/modificar/{id}").permitAll()
                        //.requestMatchers("/products/**").hasAnyRole("ADMIN", "USER")
                        .requestMatchers("/hotel/nuevo","/habitacion/nuevo").permitAll()
                        .requestMatchers("/habitacion/crear").permitAll()
                        .requestMatchers("/habitacion/listar","/hotel/listar").permitAll()
